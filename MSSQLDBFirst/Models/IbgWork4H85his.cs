using System;
using System.Collections.Generic;

namespace MSSQLDBFirst.Models
{
    public partial class IbgWork4H85his
    {
        public string FileId { get; set; }
        public string DataType { get; set; }
        public string BranCodeService { get; set; }
        public string Ediservcode { get; set; }
        public string OperationType { get; set; }
        public string YearMonth { get; set; }
        public string RcptDate { get; set; }
        public string RcptNo { get; set; }
        public string ProdSn { get; set; }
        public string CustomerCode { get; set; }
        public string CustomerType { get; set; }
        public string RcptMethod { get; set; }
        public string ProdName { get; set; }
        public string ProdCode4Sales { get; set; }
        public string CateCode { get; set; }
        public string WarrantyPeriod { get; set; }
        public string OutSoucing { get; set; }
        public string DisRepairReason { get; set; }
        public string Srflag { get; set; }
        public string Srcomment { get; set; }
        public string CurrencyType { get; set; }
        public string DiscountAmt { get; set; }
        public string TroublePhenomenonCode1 { get; set; }
        public string TroubleSpotCode1 { get; set; }
        public string TroubleReasonCode1 { get; set; }
        public string TroublePhenomenonCode2 { get; set; }
        public string TroubleSpotCode2 { get; set; }
        public string TroubleReasonCode2 { get; set; }
        public string TroublePhenomenonCode3 { get; set; }
        public string TroubleSpotCode3 { get; set; }
        public string TroubleReasonCode3 { get; set; }
        public string ReRepairType { get; set; }
        public string ExchangeReason { get; set; }
        public string PartCode1 { get; set; }
        public string Qty1 { get; set; }
        public string PartCode2 { get; set; }
        public string Qty2 { get; set; }
        public string PartCode3 { get; set; }
        public string Qty3 { get; set; }
        public string PartCode4 { get; set; }
        public string Qty4 { get; set; }
        public string PartCode5 { get; set; }
        public string Qty5 { get; set; }
        public string PartCode6 { get; set; }
        public string Qty6 { get; set; }
        public string PartCode7 { get; set; }
        public string Qty7 { get; set; }
        public string PartCode8 { get; set; }
        public string Qty8 { get; set; }
        public string PartCode9 { get; set; }
        public string Qty9 { get; set; }
        public string PartCode10 { get; set; }
        public string Qty10 { get; set; }
        public string PartCode11 { get; set; }
        public string Qty11 { get; set; }
        public string PartCode12 { get; set; }
        public string Qty12 { get; set; }
        public string PartCode13 { get; set; }
        public string Qty13 { get; set; }
        public string PartCode14 { get; set; }
        public string Qty14 { get; set; }
        public string PartCode15 { get; set; }
        public string Qty15 { get; set; }
        public string RepairStartDate { get; set; }
        public string RepairFinishedDate { get; set; }
        public string DeliveryTime { get; set; }
        public string ReleaseCount { get; set; }
        public string RepairRank { get; set; }
        public string LaborCharge { get; set; }
        public string PartFee { get; set; }
        public string RepairTime { get; set; }
        public string RepairPerson { get; set; }
        public string Rcptor { get; set; }
        public string UpdateDate { get; set; }
        public string Reverse { get; set; }
        public DateTime? CreateDate { get; set; }
        public string RecordId { get; set; }
    }
}
